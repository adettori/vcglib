...
public:
    ...
    GaussianSplat(vcg::Quaternion<ScalarType> rot, vcg::Point3<ScalarType> scale,
        ...
        if(DegreeSH > 0)
        {
            this->sphR = vcg::math::SphericalHarmonics<float,DegreeSH+1>::Wrap(&vecSHR[1]);
            this->sphG = vcg::math::SphericalHarmonics<float,DegreeSH+1>::Wrap(&vecSHG[1]);
            this->sphB = vcg::math::SphericalHarmonics<float,DegreeSH+1>::Wrap(&vecSHB[1]);
        }
    ...
    {
        return vcg::Color4b(fdcToColor(this->colorValues[0] + sphR(theta, phi)), // r
                            fdcToColor(this->colorValues[1] + sphG(theta, phi)), // g
                            fdcToColor(this->colorValues[2] + sphB(theta, phi)), // b
                            this->colorValues[3]);
    }
